...
void NIRScanner::config_from_GUI(
    uint16_t wavelength_start_nm, // Minimum wavelength to start the scan from, in nm.
    uint16_t wavelength_end_nm, // Maximum wavelength to end the scan at, in nm.
    uint8_t scan_type,  // 0: COLUMN_TYPE, 1: HADAMARD_TYPE, 2: SLEW_TYPE.
    uint16_t num_repeats, // Number of times to repeat the scan on the spectromter before averaging the scans together and returning the results. This can be used to increase integration 
    uint8_t width_px // Pixel width of the patterns. Increasing this will increase SNR, but reduce resolution.) {
)
{
    this->mConfig.scanCfg.wavelength_start_nm = wavelength_start_nm;
 ...
 * saves the scan results in .csv and .bat files ina user settings directory
 * displays the spectrum - plots the scan dat on the GUI
 */
{
    void* pData;
    int scanStatus;
    ...
    for (int i = 0; i < 288; i++) {
        wavelengthnir[i] = this->mScanResults.wavelength[i];
    }
    for (int i = 0; i < 288; i++) {
        intensitynir[i] = this->mScanResults.intensity[i];
    }
}

void NIRScanner::copyData()
{
    //Sao chép mảng
    for (int i = 0; i < 288; i++) {
        wavelengthnir[i] = this->mScanResults.wavelength[i];
    ...
    //printf("Mang nguon: \n");
    //for (int i = 0; i < 228; i++) {
    //    printf("%6.4f \t", this->mScanResults.wavelength[i]);
    //    printf("%5d \t", this->mScanResults.intensity[i]);
    //}

    //printf("\nmang dich:\n");
    //for (int i = 0; i < 228; i++) {
    //    printf("%6.4f \t", wavelengthnir[i]);
    //    printf("%5d \t", intensitynir[i]);
    //    //printf("%.2f \t", absorbance[i]);
    //}
}
...
void NIRScanner::set_element_wavelengthnir(int index, int value) {
    wavelengthnir[index] = value;
}

void NIRScanner::set_element_intensitynir(int index, int value) {
    intensitynir[index] = value;
}

float NIRScanner::get_element_wavelengthnir(int index) {
    return wavelengthnir[index];
}
int  NIRScanner::get_element_intensitynir(int index) {
    return intensitynir[index];
}
